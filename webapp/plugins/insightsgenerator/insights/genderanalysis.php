				...
				foreach ( $posts as $post ) {
					...
					if ($female > $male) {
						$this->insight_dao->insertInsightDeprecated ( 'gender_analysis', $instance->id,
							...
							" times women</strong> interested in " . $instance->network_username . "'s post",
							$filename, Insight::EMPHASIS_HIGH, serialize ( array ($post, $gender_data) ) );
					} elseif ($male > $female) {
						$this->insight_dao->insertInsightDeprecated ( 'gender_analysis', $instance->id,
							$simplified_post_date, "Men favorite!", "<strong>" . number_format ( $male ) .
							" times men</strong> interested in  " . $instance->network_username . "'s post",
							$filename, Insight::EMPHASIS_HIGH, serialize ( array ($post, $gender_data) ) );
					} else {
						$this->insight_dao->insertInsightDeprecated ( 'gender_analysis', $instance->id,
							$simplified_post_date, "Loved by all!", "<strong>" . number_format ( $female + $male ) .
							" times women and men </strong> interested in  " . $instance->network_username .
							"'s post", $filename, Insight::EMPHASIS_HIGH, serialize ( array ($post, $gender_data) ) );
					}
				}
