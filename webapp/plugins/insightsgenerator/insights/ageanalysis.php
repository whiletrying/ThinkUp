				...
				foreach ( $posts as $post ) {
					...
					if (max ( $age_data ) == $age_data ['18']) {
						$this->insight_dao->insertInsightDeprecated ( 'age_analysis', $instance->id,
							$simplified_post_date, "Teens!", "<strong>" . number_format ( $age_data ['18'] ) .
							" times people</strong> at the age less than 18 years interested in " .
						...
						$this->insight_dao->insertInsightDeprecated ( 'age_analysis', $instance->id,
							$simplified_post_date, "So young!", "<strong>" . number_format ( $age_data ['18_25'] ) .
							 " times people</strong> aged 18 to 25 years interested in " .
							$instance->network_username . "'s post", $filename, Insight::EMPHASIS_HIGH,
							serialize ( array ($post, $age_data ) ) );
					} elseif (max ( $age_data ) == $age_data ['25_35']) {
						$this->insight_dao->insertInsightDeprecated ( 'age_analysis', $instance->id,
							$simplified_post_date, "Oh, adults!", "<strong>" . number_format ( $age_data ['25_35'] )
							 . " times people</strong> aged 25 to 35 years interested in " .
							$instance->network_username . "'s post", $filename, Insight::EMPHASIS_HIGH,
							serialize ( array ($post, $age_data ) ) );
					} elseif (max ( $age_data ) == $age_data ['35_45']) {
						$this->insight_dao->insertInsightDeprecated ( 'age_analysis', $instance->id,
							$simplified_post_date, "Middle-aged!", "<strong>" .number_format ( $age_data ['35_45'] )
							 . " times people</strong>aged 35 to 45 years interested in " .
							$instance->network_username . "'s post", $filename, Insight::EMPHASIS_HIGH,
							serialize ( array ($post, $age_data ) ) );
					} elseif (max ( $age_data ) == $age_data ['45']) {
						$this->insight_dao->insertInsightDeprecated ( 'age_analysis', $instance->id,
							$simplified_post_date, "Seniors!", "<strong>" . number_format ( $age_data ['45'] ) .
							 " times people</strong> at the age more than 45 years interested in " .
							$instance->network_username . "'s post", $filename, Insight::EMPHASIS_HIGH,
							serialize ( array ($post, $age_data ) ) );
					}
				}
